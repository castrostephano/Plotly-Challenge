...
function buildDemo() {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var filteredData = sampledata.filter(subjects => subjects.id.toString() === id)[0];

    // add them to the dropdown
    var selection = d3.select("#sample-metadata")
    Object.entries(filteredData).forEach((key) => 
    {selections.append("h5").text(key[0].toUpperCase() + ": " + key[1] + "\n");

      // data.metadata.forEach(function (name) {
